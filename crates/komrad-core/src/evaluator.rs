                ...
                match (target_val, index_val) {
                    (Value::List(vs), Value::Int(i)) => {
                        if i >= 0 && (i as usize) < vs.len() {
                            ...
                            Value::Error(RuntimeError::ArgumentError("Index out of bounds".to_string()).as_spanned(
                                index.span.clone()
                            ))
                        }
                    }
...
mod tests {
    ...
    async fn test_evaluate_binary_divide_by_zero() {
        ...
        let result = expr.evaluate(&mut context).await;
        match result {
            Value::Error(Spanned { value: box RuntimeError::ArgumentError(msg), .. }) => {
    ...
    async fn test_evaluate_slice_expr_out_of_bounds_get_simulated_spanned_info() {
        ...
        let result = expr.evaluate(&mut context).await;
        match result {
            Value::Error(Spanned { span, value: box RuntimeError::ArgumentError(msg), .. }) => {
