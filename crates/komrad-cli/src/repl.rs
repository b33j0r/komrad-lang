...
use tokio::{sync::mpsc as async_mpsc, time};
use tracing::debug;
...
/// Simulated interpreter that parses the input and returns a result after a delay.
async fn interpret_input(input: &str) -> Result<String, Box<dyn Error>> {
    ...
    debug!("Parsed Statement as SExpr: {:#?}", sexpr);
...
pub async fn main() -> Result<(), Box<dyn Error>> {
    ...
    input_area.set_style(Style::default().fg(Color::White));
